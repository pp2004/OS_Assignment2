...
void max_heapify(int arr[][2], int n, int i) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
...
void build_max_heap(int arr[][2], int n) {
    for (int i = n / 2 - 1; i >= 0; i--)
        max_heapify(arr, n, i);
}

// Extract max from heap
void extract_max(int arr[][2], int* n) {
    if (*n <= 0) return;
...
{
    if (ship.waitingTime >= 0) return 0; // Ship not docked yet
    
    // Check if ship has any cargo left
    int ship_cargo_is_empty = 1;
    ...
    if (!ship_cargo_is_empty) {
        // Build max heaps for both cargo and cranes
        int active_cargos = 0;
        int ship_cargo[ship.numCargo][2];
        
        // Only include non-zero cargo
        for (int i = 0; i < ship.numCargo; i++) {
            if (ship.cargo[i] != 0) {
                ship_cargo[active_cargos][0] = ship.cargo[i]; // Cargo value
                ship_cargo[active_cargos][1] = i;             // Original index
                active_cargos++;
            }
        }

        int active_cranes = dock_cap;
        int sorted_crane_cap[dock_cap][2];
        for (int i = 0; i < dock_cap; i++) {
            sorted_crane_cap[i][0] = crane_capacity[i]; // Capacity value
            sorted_crane_cap[i][1] = i;                 // Original index
        }
        
        // Build max heaps - O(n) operations
        build_max_heap(ship_cargo, active_cargos);
        build_max_heap(sorted_crane_cap, active_cranes);
        
        // Match cargos to cranes using greedy approach
        while (active_cargos > 0 && active_cranes > 0) {
            // If largest crane can handle largest cargo
            if (sorted_crane_cap[0][0] >= ship_cargo[0][0]) {
                // Create message for cargo handling
                ...
                handle_cargo.cargoId = ship_cargo[0][1];
                handle_cargo.craneId = sorted_crane_cap[0][1];
                
                // Mark cargo as handled
                ...
                // Remove used crane and cargo from heaps
                extract_max(sorted_crane_cap, &active_cranes);
                extract_max(ship_cargo, &active_cargos);
            } else {
                // Largest crane can't handle largest cargo, so this cargo can't be handled
                extract_max(ship_cargo, &active_cargos);
            }
        }
    } else {
